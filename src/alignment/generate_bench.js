...
function createAlignment(jsonObj, store) {
  ...
  const rotations = 3000;
  const total = jsonObj.length * rotations;

  jsonObj.forEach((o, index) => {
    console.log('progress ' + ((index * jsonObj.length) * rotations / total) + '%');
    for (let i = 0; i < rotations; i++) {
      o.GREL = o.GREL.split('_')[0] + '_' + i;
      o.SPARQL = o.SPARQL.split('_')[0] + '_' + i;
      o.XPath = o.XPath.split('_')[0] + '_' + i;
      o.SQL = o.SQL.split('_')[0] + '_' + i;
      if (!o.function) {
        return;
      }
  ...
  if (o.GREL_signature) {
    try {
      ...
      fnObj.parameters.forEach((p, i) => {
        ...
        const param = node('fns', `Parameter/grel_${o.GREL}_${p.type}_${encodeURIComponent(p.argument)}`);
        store.addQuad(param, nodes.a, node('fno', 'Parameter'));
        store.addQuad(param, node('fno', 'predicate'), node('fns', `Predicate/grel_${p.type}_${encodeURIComponent(p.argument)}`));
  ...
  if (o.SPARQL_signature) {
    ...
    fns.forEach((fnObj, index) => {
      ...
      fnObj.parameters.forEach((p, i) => {
        ...
        store.addQuad(param, nodes.a, node('fno', 'Parameter'));
        store.addQuad(param, node('fno', 'predicate'), node('fns', `Predicate/sparql_${p.type}_${p.argument}`));
        store.addQuad(param, node('fno', 'type'), typeToRealType(p.type));
        store.addQuad(param, node('rdfs', 'label'), literal(p.argument));
        parameters.push(param);
        store.addQuad(paramMapping, node('fnom', 'functionParameter'), param);
...
function doXpath(o, store, problem, fn, implementation) {
  ...
  if (o.Xpath_signature) {
    ...
    fns.forEach((fnObj, index) => {
      ...
      const parameters = [];
      fnObj.parameters.forEach((p, i) => {
        const paramMapping = node('fns', `ParameterMapping/xpath_${o.XPath}_${encodeURIComponent(p.type)}_${p.argument}`);
        store.addQuad(mapping, node('fno', 'parameterMapping'), paramMapping);
        store.addQuad(paramMapping, nodes.a, node('fno', 'ParameterMapping'));
        store.addQuad(paramMapping, nodes.a, node('fnom', 'PositionParameterMapping'));
        store.addQuad(paramMapping, node('fnom', 'implementationParameterPosition'), literal(i));
        ...
        const param = node('fns', `Parameter/xpath_${o.XPATH}_${encodeURIComponent(p.type)}_${encodeURIComponent(p.argument)}`);
        store.addQuad(param, nodes.a, node('fno', 'Parameter'));
        store.addQuad(param, node('fno', 'predicate'), node('fns', `Predicate/xpath_${encodeURIComponent(p.argument)}`));
        store.addQuad(param, node('fno', 'type'), typeToRealType(p.type));
        store.addQuad(param, node('rdfs', 'label'), literal(p.argument));
        parameters.push(param);
        store.addQuad(paramMapping, node('fnom', 'functionParameter'), param);
  ...
  if (o.SQL_signature) {
    try {
      ...
      fnObj.parameters.forEach((p, i) => {
        ...
        store.addQuad(param, nodes.a, node('fno', 'Parameter'));
        store.addQuad(param, node('fno', 'predicate'), node('fns', `Predicate/sql_${p.type}_${p.argument}`));
